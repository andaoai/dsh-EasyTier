// dsh-EasyTier · Client 半部分（Cordis 动态插件函数体）
// 在「设置」中注册 EasyTier 组网页：状态总览、权限提示、对等节点表格、
// 启动/停止/重启与引导节点管理。纯 React.createElement，无 JSX。
//
// 说明：本文件内容即 cordis_define 的 code.client「函数体」。

return {
  name: 'dsh-easytier-client',
  inject: ['timer'],
  apply(ctx) {
...
`
    ...
    function Section() {
      ...
      async function doBootstrap() {
        const r = await withBusy('boot', async () => {
      ...
      const field = (label, node, sub) => h('div', null,
        h('label', { className: 'etx-lbl' }, label),
        node,
        sub ? h('div', { className: 'etx-sub' }, sub) : null,
      )
      ...
      if (capsBad) {
        kids.push(h('div', { className: 'etx-warn' },
          ...
          h('div', { style: { opacity: 0.75 } }, '执行后回到本页点击「启动」即可。wfmon 采用的是同样的 ambient capabilities 方式。'),
        ))
      }
      ...
      kids.push(h('div', { className: 'etx-foot' },
        '实例由本插件管理，DSH 进程退出或插件停止时会一并停止；需要长期常驻请用 easytier-core --daemon 或 systemd。',
      ))
    ...
    ctx.effect(() => {
      ...
      return () => { try { disposeStyle() } catch (e) {} try { disposeSlot() } catch (e) {} }
    })
  },
}
